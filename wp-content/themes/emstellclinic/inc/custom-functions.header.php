...
/**
Function name: 				clinika_get_nav_menu()			
Function description:		Get page NAV MENU
*/
function clinika_get_nav_menu(){
    ...
    if ( has_nav_menu( 'primary' ) ) {
		...
		if( current_user_can('administrator') ) {
			$html .= '<p class="no-menu text-right">'.esc_html__('Primary navigation menu is missing.','clinika').'</p>';
		}
	}
...
*/
if (!function_exists('clinika_get_theme_logo')) {
	function clinika_get_theme_logo(){
		...
		if (is_page() || is_single() || is_404() || is_archive() || is_search() || is_home()) {
			...
			if ($custom_header_activated && $custom_header_activated == 'yes') {
				...
				if (clinika_redux('clinika_logo','url')) {
					$html .= '<img class="theme-logo main-static-logo" src="'.esc_url(clinika_redux('clinika_logo','url')).'" alt="'.esc_attr(get_bloginfo()).'" />';
				}else{
					$html .= get_bloginfo();
				}
			}
...
if (!function_exists('clinika_get_theme_logo_sticky')) {
	function clinika_get_theme_logo_sticky(){

		$html = '';

    	if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
			if (clinika_redux('clinika_logo_sticky_header','url') != '' && clinika_redux('is_nav_sticky') != false) {
				$html .= '<img class="theme-logo theme-logo-sticky" src="'.esc_url(clinika_redux('clinika_logo_sticky_header','url')).'" alt="'.esc_attr(get_bloginfo()).'" />';
			}else{
				$html .= '<img class="theme-logo theme-logo-sticky" src="'.esc_url(get_template_directory_uri().'/images/theme_clinika_logo_dark.png').'" alt="'.esc_attr(get_bloginfo()).'" />';
			}
		}
...
if (!function_exists('clinika_get_login_link')) {
    function clinika_get_login_link(){
        ...
        <a href="<?php echo esc_url($user_url); ?>" <?php echo $data_attributes; ?>>
            <?php esc_html_e('Sign In','clinika'); ?>
        </a>

        <?php 
    }
    add_action('clinika_login_link_a', 'clinika_get_login_link');
}
...
*/
add_action('wp_enqueue_scripts', 'clinika_dynamic_css' );
function clinika_dynamic_css(){

    global  $clinika_redux;
    $html = '';

	wp_enqueue_style(
	   'clinika-custom-style',
	...
	// BORDER RADIUS STYLE
    if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
    	if (clinika_redux("clinika_border_radius") == 'default') {
			$btn_border_radius = '30px';
    	} else if (clinika_redux("clinika_border_radius") == 'boxed') {
    		$btn_border_radius = '0px';
    	}  else if (clinika_redux("clinika_border_radius") == 'round') {
    ...
    }elseif (clinika_redux('mt_preloader_status')) {
        $html .= 'body .linify_preloader_holder{
					background-color: '.clinika_redux('mt_preloader_status').';
        		}';
    }
  
    // THEME OPTIONS STYLESHEET
    if(!clinika_redux('breadcrumbs-delimitator')) {
    	$delimitator = '/';
    } else {
    	$delimitator = clinika_redux('breadcrumbs-delimitator');
    }
    ...
    if ( is_page()) {
		if ($thumbnail_src) {
	    	...
	    	if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
	    		if (clinika_redux('clinika_header_breadcrumbs_image','url') != '') {
		        	$html .= '.modeltheme-breadcrumbs {background-image:url("'.esc_url(clinika_redux('clinika_header_breadcrumbs_image','url')).'");}';
	    		}else{
		        	$html .= '.modeltheme-breadcrumbs {background-color:#2C3E50;}';
	    		}
	        }else{
		        $html .= '.modeltheme-breadcrumbs {background-color:#2C3E50;}';
	        }
    ...
    // Defaults
	$skin_main_bg = '#F04237';
	$skin_main_bg_hover = '#D02F23';
	$skin_main_texts_color = '#F04237';
    if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
    	if (clinika_redux("mt_style_main_backgrounds_color") != '') {
			$skin_main_bg = clinika_redux("mt_style_main_backgrounds_color");
    	}

    	if (clinika_redux("mt_style_main_backgrounds_color_hover") != '') {
			$skin_main_bg_hover = clinika_redux("mt_style_main_backgrounds_color_hover");
    	}

    	if (clinika_redux("mt_style_main_texts_color") != '') {
			$skin_main_texts_color = clinika_redux("mt_style_main_texts_color");
    	}
   	}
    ...
    if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
	    $html .= '.single article .article-content p,
	               ...
	               dd,
	               address{
	                    font-family: '.clinika_redux('modeltheme-blog-post-typography','font-family').';
	               }
	               h1,
	               h1 span {
	                    font-family: "'.clinika_redux('modeltheme-heading-h1','font-family').'";
	                    font-size: '.clinika_redux('modeltheme-heading-h1','font-size').';
	               }
	               h2 {
	                    font-family: "'.clinika_redux('modeltheme-heading-h2','font-family').'";
	                    font-size: '.clinika_redux('modeltheme-heading-h2','font-size').';
	               }
	               h3 {
	                    font-family: "'.clinika_redux('modeltheme-heading-h3','font-family').'";
	                    font-size: '.clinika_redux('modeltheme-heading-h3','font-size').';
	               }
	               h4 {
	                    font-family: "'.clinika_redux('modeltheme-heading-h4','font-family').'";
	                    font-size: '.clinika_redux('modeltheme-heading-h4','font-size').';
	               } 
	               h5 {
	                    font-family: "'.clinika_redux('modeltheme-heading-h5','font-family').'";
	                    font-size: '.clinika_redux('modeltheme-heading-h5','font-size').';
	               } 
	               h6 {
	                    font-family: "'.clinika_redux('modeltheme-heading-h6','font-family').'";
	                    font-size: '.clinika_redux('modeltheme-heading-h6','font-size').';
	               } 
	               #navbar .menu-item > a {
	                    font-family: "'.clinika_redux('modeltheme-navigation-typography','font-family').'";
	                    font-weight: '.clinika_redux('modeltheme-navigation-typography','font-weight').';
	               } 
	               input,
	               textarea {
	                    font-family: '.clinika_redux('modeltheme-inputs-typography','font-family').';
	               }
					...
					.woocommerce .woocommerce-message .button,
	               	input[type="submit"] {
	                    font-family: '.clinika_redux('modeltheme-buttons-typography','font-family').';
	               	}';
	    ...
	    $html .= '
	    			@media only screen and (max-width: 767px) {
	    				body h1,
	    				body h1 span{
	    					font-size: '.clinika_redux('mt_heading_h1_smartphones', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h1_smartphones', 'line-height').' !important;
	    				}
	    				body h2{
	    					font-size: '.clinika_redux('mt_heading_h2_smartphones', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h2_smartphones', 'line-height').' !important;
	    				}
	    				body h3{
	    					font-size: '.clinika_redux('mt_heading_h3_smartphones', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h3_smartphones', 'line-height').' !important;
	    				}
	    				body h4{
	    					font-size: '.clinika_redux('mt_heading_h4_smartphones', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h4_smartphones', 'line-height').' !important;
	    				}
	    				body h5{
	    					font-size: '.clinika_redux('mt_heading_h5_smartphones', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h5_smartphones', 'line-height').' !important;
	    				}
	    				body h6{
	    					font-size: '.clinika_redux('mt_heading_h6_smartphones', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h6_smartphones', 'line-height').' !important;
	    				}
	    			}';
	    ...
	    $html .= '
	    			@media only screen and (min-width: 768px) and (max-width: 1024px) {
	    				body h1,
	    				body h1 span{
	    					font-size: '.clinika_redux('mt_heading_h1_tablets', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h1_tablets', 'line-height').' !important;
	    				}
	    				body h2{
	    					font-size: '.clinika_redux('mt_heading_h2_tablets', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h2_tablets', 'line-height').' !important;
	    				}
	    				body h3{
	    					font-size: '.clinika_redux('mt_heading_h3_tablets', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h3_tablets', 'line-height').' !important;
	    				}
	    				body h4{
	    					font-size: '.clinika_redux('mt_heading_h4_tablets', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h4_tablets', 'line-height').' !important;
	    				}
	    				body h5{
	    					font-size: '.clinika_redux('mt_heading_h5_tablets', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h5_tablets', 'line-height').' !important;
	    				}
	    				body h6{
	    					font-size: '.clinika_redux('mt_heading_h6_tablets', 'font-size').' !important;
	    					line-height: '.clinika_redux('mt_heading_h6_tablets', 'line-height').' !important;
	    				}
	    			}';
   	}
	...
	$html .= '.breadcrumb a::after {
		    ...
		    .navbar-header .logo img {
		        max-width: '.clinika_redux('logo_max_width').'px;
		    }

		    ::selection{
		        color: '.clinika_redux('mt_text_selection_color').';
		        background: '.clinika_redux('mt_text_selection_background_color').';
		    }
		    ...
		    .wishlist_table tr td.product-stock-status span.wishlist-in-stock {
		        color: '.clinika_redux('mt_global_link_styling', 'regular').';
		    }
			...
			.woocommerce ul.products li.product a:hover {
		        color: '.clinika_redux('mt_global_link_styling', 'regular').' !important;
		    }
			...
			.woocommerce ul.products li.product .button:hover {
		    	color: '.clinika_redux('mt_global_link_styling', 'hover').' !important;
		    }
		    a:hover,
		    .is_header_semitransparent #navbar ul.menu .buy-button a:hover {
		    	color: '.clinika_redux('mt_global_link_styling', 'hover').';
		    }
		    #sln-salon h1 {
		    	font-family: "'.clinika_redux('modeltheme-heading-h1','font-family').'" !important;
			}
		    /*------------------------------------------------------------------
		        COLOR
		    ------------------------------------------------------------------*/
			...
			.comment-edit-link:hover, .comment-reply-link:hover  {
		        background: '.clinika_redux("mt_style_semi_opacity_backgrounds", "color").';
		    }
		    .mt-tabs .tabs nav li.tab-current a {
		    	background: linear-gradient(90deg, '.esc_html($skin_main_bg).' 0%, '.esc_html($skin_main_bg_hover).' 100%) !important;
		    }
    ...
    wp_add_inline_style( 'clinika-custom-style', clinika_minify_css($html) );
}
...
*/
function clinika_get_site_fonts(){
    global  $clinika_redux;
    $fonts_string = 'Jost:regular,300,400,500,600,700,bold%7C';
    if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
	    if (isset($clinika_redux['google-fonts-select'])) {
	        $i = 0;
	        $len = count($clinika_redux['google-fonts-select']);
	        foreach(array_keys($clinika_redux['google-fonts-select']) as $key){
	            $font_url = str_replace(' ', '+', $clinika_redux['google-fonts-select'][$key]);
	        ...
	        // enqueue fonts
	        wp_enqueue_style( 'clinika-fonts', $fonts_url, array(), '1.0.0' );
	    }
	} else {
        ...
        wp_enqueue_style( 'clinika-fonts-fallback', $fonts_url, array(), '1.0.0' );
    }
}
add_action('wp_enqueue_scripts', 'clinika_get_site_fonts');


/**
||-> FUNCTION: Add specific CSS class by filter
*/
add_filter( 'body_class', 'clinika_body_classes' );
function clinika_body_classes( $classes ) {

    // CHECK IF FEATURED IMAGE IS FALSE(Disabled)
    $post_featured_image = '';
    if (is_singular('post')) {
        if (clinika_redux('post_featured_image')) {
            if (clinika_redux('post_featured_image') == false) {
    ...
    $is_nav_sticky = '';
    if (clinika_redux('is_nav_sticky')) {
        if (clinika_redux('is_nav_sticky') == true) {
            // If is sticky
