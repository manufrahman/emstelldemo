...
$image_size = 'clinika_700x600';
if ( class_exists( 'ReduxFrameworkPlugin' ) ) {
    if ( clinika_redux('blog-display-type') == 'list' ) {
        $type_class = 'grid-three-columns';
        $image_size = 'clinika_700x600';
    } else {
        $type_class = 'grid-view';
        if ( clinika_redux('blog-grid-columns') == 1 ) {
            $master_class = 'col-md-12';
            $type_class .= ' grid-one-column';
            $image_size = 'clinika_1000x580';
        }elseif ( clinika_redux('blog-grid-columns') == 2 ) {
            $master_class = 'col-md-6';
            $type_class .= ' grid-two-columns';
            $image_size = 'clinika_700x500';
        }elseif ( clinika_redux('blog-grid-columns') == 3 ) {
            $master_class = 'col-md-4';
            $type_class .= ' grid-three-columns';
            $image_size = 'clinika_700x500';
        }elseif ( clinika_redux('blog-grid-columns') == 4 ) {
        ...
        <div class="<?php echo esc_attr($post_col); ?> post-details">
            ...
            <h3 class="post-name row">
                <a title="<?php the_title_attribute() ?>" href="<?php esc_url(the_permalink()); ?>">
                    <?php the_title() ?><?php if( is_sticky() ) { echo esc_html__('*', 'clinika'); } ?>
                </a>
            </h3>
