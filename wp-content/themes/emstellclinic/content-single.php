       ...
       <div class="row">
            ...
            if ( function_exists('modeltheme_framework')) {
                if (isset($select_post_sidebar) && $select_post_sidebar != '') {
                    ...
                    $sidebar = clinika_redux('clinika_blog_layout_sidebar');
                }
            }
            ...
            if (isset($select_post_layout) && in_array($select_post_layout, $sidebars_lr_meta)) {
                ...
                $sidebars_lr_panel = array("clinika_blog_left_sidebar", "clinika_blog_right_sidebar");
                if (in_array(clinika_redux('clinika_single_blog_layout'), $sidebars_lr_panel)) {
            ...
            <?php if((class_exists( 'ReduxFrameworkPlugin' ))) {
                if ( clinika_redux('clinika_single_blog_layout') == 'clinika_blog_left_sidebar' && is_active_sidebar( $sidebar )) { ?>
            ...
            <?php $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ),'clinika_blog_900x550' ); 
            ...
            <div class="<?php echo esc_attr($cols); ?> <?php echo esc_attr($content_spacing_class); ?> main-content">
                ...
                <div class="article-header article-header-details">
                    <?php if($thumbnail_src) { ?>
                        <?php the_post_thumbnail( 'clinika_blog_900x550' ); ?>
                    <?php } ?>
                    ...
                    <div class="article-details">
                        ...
                        <div class="post-author">
                            <span><i class="fa fa-user-o" aria-hidden="true"></i><?php echo esc_html__('Posted by ', 'clinika') . '<a href="'.esc_url(get_author_posts_url( get_the_author_meta( 'ID' ), get_the_author_meta( 'user_nicename' ) )).'"><strong>' . esc_html(get_the_author()) . '</strong></a>'; ?></span> 
                            <span><?php echo get_the_term_list( get_the_ID(), 'category', esc_html__('in ', 'clinika'), ', ' ); ?></span>
                        </div>
                    </div>
                </div>
            ...
            <?php if ( clinika_redux('clinika_single_blog_layout') == 'clinika_blog_right_sidebar' && is_active_sidebar( $sidebar )) { ?>
            <div class="col-md-4 sidebar-content">
                <?php if ( is_active_sidebar ( $sidebar ) ) { ?>
                    <?php  dynamic_sidebar( $sidebar ); ?>
                <?php } ?>
    ...
    <div class="container">
        <div class="">
            <?php if ( class_exists( 'ReduxFrameworkPlugin' ) && function_exists('modeltheme_framework') ) { ?>
                <?php if ( clinika_redux('modeltheme-enable-related-posts') ) { ?>
                    <div class="related-posts sticky-posts col-md-12">
                        <h2 class="heading-bottom"><?php esc_html_e('Related Posts', 'clinika'); ?></h2>
                        <div class="row">
                        <?php 
                            $args=array(  
                                ...
                                <div class="col-md-4 post">
                                    <div class="related_blog_custom">
                                        <a href="<?php esc_url(the_permalink()); ?>" class="relative">
                                            <?php
                                            $thumbnail_src2 = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'clinika_related_post_pic500x300'); 
                                        ...
                                        <div class="related_blog_details <?php echo esc_attr($missing_image_class); ?>">
                                            ...
                                            <div class="post-excerpt row">
                                                <?php echo strip_tags(clinika_excerpt_limit($post->post_content, 8)); ?>
                                            </div>
                                            <div class="clearfix"></div>
                                            <div class="post-read-more">
                                                <a class="rippler rippler-default" href="<?php esc_url(the_permalink()); ?>"><?php echo esc_html__('Read More','clinika')?> <i class="fa fa-angle-right" aria-hidden="true"></i></a>
                                            </div>
                                            <div class="clearfix"></div>
                                        </div>
                                    </div>
