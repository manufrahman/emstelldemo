...
function modeltheme_skills_shortcode($params, $content) {
    ...
    $image_skill      = wp_get_attachment_image_src($image_skill, "clinika_500x500");
...
/*--- 14. Pricing tables ---*/
/*---------------------------------------------*/
function clinika_pricing_table_shortcode($params, $content) {
    extract( shortcode_atts( 
        array(
            'package_currency'  => '',
            'package_price'     => '',
...
add_shortcode('pricing-table-v2', 'clinika_pricing_table_shortcode');
...
function modeltheme_btn_shortcode($params, $content) {
    ...
    $content .= '<div class="'.$align.' modeltheme_button mt_modeltheme_button '.$clss.' animateIn" data-animate="'.$animation.'" data-identificator="'.$clss.'" data-background-color-hover="'.$color_hover.'" data-background-color="'.$color.'" data-text-color-hover="'.$hover_text_color.'">';
        $content .= '<a href="'.esc_url($btn_url).'" class="rippler rippler-default button-winona '.$btn_size.'" style="color:'.$text_color.';">'.$btn_text.'</a>';
    $content .= '</div>';
    return $content;
}
add_shortcode('clinika_btn', 'modeltheme_btn_shortcode');
...
function modeltheme_call_to_action_shortcode( $params, $content ) {
    ...
    $shortcode_content = '<div class="clinika_call-to-action">';
...
function modeltheme_service_activity_shortcode ( $params, $content ) {
    ...
    $shortcode_content .= '<div class="activities row animateIn" data-animate="'.esc_attr($animation).'">';
        ...
        foreach ($recentposts as $post) {
            $counter++;
            $activities_icon = get_post_meta( $post->ID, 'clinika_service_badge_icon', true );
            $activities_badge_background_color = get_post_meta( $post->ID, 'clinika_service_badge_background_color', true );
            ...
            $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ),'clinika_500x500' );
                    ...
                    $shortcode_content .= '<div class="activities-content row">';
                        ...
                        $shortcode_content .= '<div class="shop_feature_v3">';
                            ...
                            $shortcode_content .= '<div class="pull-left vc_col-md-9 shop_feature_description_v3">';
                                $shortcode_content .= '<a href="'.get_permalink($post->ID).'" class="shop_feature_readmore_v3"><h3 class="shop_feature_heading_v3">'.esc_attr($activities_service_title).'</h3></a>';
                                $shortcode_content .= '<p class="shop_feature_subheading_v3">'.strip_tags(clinika_excerpt_limit($content,16)).' ...</p>';
                            $shortcode_content .= '</div>';
                            $shortcode_content .= '</div>';
                    ...
                    $shortcode_content .= '<div class="activities-content row">';
                        ...
                        $shortcode_content .= '<div class="shop_feature_v3">';
                            ...
                            $shortcode_content .= '<div class="pull-left vc_col-md-12 shop_feature_description_v3">';
                                $shortcode_content .= '<h3 class="shop_feature_heading_v3"><a href="'.get_permalink($post->ID).'">'.esc_attr($activities_service_title).'</a></h3>';
                                $shortcode_content .= '<p class="shop_feature_subheading_v3">'.strip_tags(clinika_excerpt_limit($content,8)).' ...</p>';
                            $shortcode_content .= '</div>';
...
function modeltheme_show_blog_post_shortcode( $params, $content ) {
    ...
    $shortcode_content = '<div class="clinika_shortcode_blog vc_row sticky-posts animateIn" data-animate="'.$animation.'">';
    $counter = 0;
    foreach ($posts as $post) {
        ...
        if ($style_type == "style_1") {
            $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ),'clinika_700x500' );
        } else if ($style_type == "style_2") {
            $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ),'clinika_500x680' );
        } else {
            $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ),'clinika_700x500' );
        }
        ...
        $post_background_color = get_post_meta( $post->ID, 'clinika_post_background_color', true );
            ...
            $shortcode_content .= '<div class="'.esc_attr($columns).' post '.$style_type.'">';
                $shortcode_content .= '<div class="col-md-12 shortcode_post_content ' .$custom_blog_color1.'">';
                    ...
                    if ($columns == 'vc_col-sm-6') {
                        ...
                        $shortcode_content .= '<div class="post-excerpt">'.strip_tags(clinika_excerpt_limit($post->post_content, 18)).' ...</div>';
                        ...
                        $shortcode_content .= '<div class="post-more-download">';
                            if ( $columns == 'vc_col-sm-3'){
                                ...
                                $shortcode_content .= '<div class="post-read-more">';
                                $shortcode_content .= '<a class="rippler rippler-default" href="'.get_permalink($post->ID).'">'.esc_html__('Read More','modeltheme').'<i class="fa fa-angle-right" aria-hidden="true"></i></a>';
                                $shortcode_content .= '</div>';
                            }
                        $shortcode_content .= '</div>';
                    $shortcode_content .= '</div>';
...
function modeltheme_row_separator_shortcode($params, $content) {
    ...
    $content = '';
    $content .= '<div class="mt-row-separator '.esc_attr( $vc_css_class ).'" style="overflow:hidden;">';
                    // TOP LEFT
                    if ($clouds_position == 'top-left') {
                        $content .= '<svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="2116.000000pt" height="133.000000pt" viewBox="0 0 2116.000000 133.000000" preserveAspectRatio="xMidYMid meet"><g transform="translate(0.000000,133.000000) scale(0.100000,-0.100000)" fill="#000000" stroke="none">
                                        <path class="st0" style="fill: '.$bg_color.'" d="M60 1326 c0 -8 228 -141 355 -207 356 -186 686 -267 968 -239 151 15
294 48 543 126 400 125 576 161 844 171 456 16 987 -106 1606 -371 71 -31 142 -56 158 -56 15 0 173 16 350 35 850 92 1666 154 2591 197 369 17 1816 17 2150 0 894 -47 1632 -129 2265 -254 133 -26 242 -43 285 -42 39 0 194 14 345 32 609 70 1024 96 1570 96 942 0 1628 -98 2440 -349 443 -136 797 -213 1194 -259 l130 -15 305 203 c852 568 1419 835 1816 853 284 13 430 -79 570 -357 79 -157 120 -194 195 -171 68 20 156 89 289 227 l131 136 0 124 0 124 -10550 0 c-5802 0 -10550 -2 -10550 -4z"/></g>
                                    </svg>';
                    // TOP RIGHT
                    }elseif ($clouds_position == 'top-right') {
                        $content .= '<svg version="1.1" id="mt_svg_cloud" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 500 86.5" style="enable-background:new 0 0 500 86.5;" xml:space="preserve">
                                        <path class="st0" style="fill: '.$bg_color.'" d="M498.5,86.5c1-2.5,1.5-5.3,1.5-8.2c0-12.5-10.2-22.7-22.7-22.7c-1.2,0-2.3,0.1-3.4,0.3
                                            c-3.8-6-10.5-9.9-18.1-9.9c-1.7,0-3.3,0.2-4.9,0.6c-4.7-10.9-15.5-18.5-28.2-18.5c-1.7,0-3.4,0.1-5.1,0.4
                                            c-7.5-14.3-22.5-24.1-39.8-24.1c-13.5,0-25.6,6-33.8,15.4c-1.5-0.3-3.1-0.4-4.8-0.4c-4.6,0-9,1.2-12.8,3.2
                                            c-6.2-7.4-15.5-12.1-25.9-12.1c-1.6,0-3.3,0.1-4.8,0.3C289.3,4.2,280.3,0,270.3,0c-14.3,0-26.6,8.6-32.1,20.9c-3.4-1.3-7.1-2-11-2
                                            c-7.5,0-14.4,2.7-19.7,7.2c-2.7-0.8-5.5-1.3-8.4-1.3c-11.7,0-21.7,7.2-26,17.3c-4.9,0.3-9.5,2-13.3,4.7c-4.3-3.2-9.6-5.1-15.4-5.1
                                            c-2.6,0-5,0.4-7.4,1.1c-5.4-9-15.2-15.1-26.4-15.1c-11.4,0-21.3,6.2-26.6,15.4c-1.6-0.3-3.3-0.5-5-0.5c-5.6,0-10.7,1.8-14.9,4.9
                                            c-5.1-3.6-11.2-5.7-17.9-5.7c-10.6,0-20,5.4-25.6,13.5C9.1,56.4,0,66.3,0,78.3c0,2.9,0.5,5.6,1.5,8.2H498.5z"/>
                                    </svg>';
                    // BOTTOM LEFT
                    }elseif ($clouds_position == 'bottom-left') {
                        $content .= '<svg width="2920" height="261" viewBox="0 0 1920 261" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M0 249.818C43 257.652 178.2 259.318 375 203.318C392.167 198.485 436.8 185.118 478 170.318C495.5 164.152 539.4 147.918 575 132.318C586.833 127.318 615 115.118 633 106.318C647.833 98.485 683.1 81.5184 705.5 76.3184C728.5 68.8184 788.5 53.5184 844.5 52.3184C872.667 51.3184 941.3 51.5184 990.5 60.3184C1001.33 62.3184 1027.6 67.4184 1046 71.8184C1061.33 75.6517 1097.2 83.9184 1118 86.3184C1147 91.3184 1216.3 101.518 1261.5 102.318C1306.7 103.118 1367.5 109.182 1460.5 83.8184C1475.17 79.8184 1514.6 68.8184 1555 56.8184C1570.17 52.3184 1610.1 40.5184 1648.5 29.3184C1667.67 22.985 1714.2 9.61836 1747 6.81836C1770.83 2.1517 1838.8 -4.78165 1920 4.81836V260.818H0V249.818Z" fill="white"/>
</svg>';
                    // BOTTOM RIGHT
                    }elseif ($clouds_position == 'bottom-right') {
                        $content .= '<svg version="1.1" id="mt_svg_cloud" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 417.9 78.2" style="enable-background:new 0 0 417.9 78.2;" xml:space="preserve">
                                        <path class="st0" style="fill: '.$bg_color.'" d="M0,27.9V0h417.9v27.5c-1.2,0.4-2.6,0.6-3.9,0.8c-5,7.4-13.5,12.2-23.1,12.2c-6,0-11.6-1.9-16.2-5.2
...
/*---------------------------------------------*/
function clinika_shop_categories_with_xsthumbnails_shortcode( $params, $content ) {
    extract( shortcode_atts( 
        array( 
            'category'                             => '',
            'overlay_color1'                       => '',
...
add_shortcode('shop-categories-with-xsthumbnails', 'clinika_shop_categories_with_xsthumbnails_shortcode');
    ...
    $html .= '<script>
       ...
       $args_prods = array(
        ...
        foreach ($blogposts as $blogpost) {
                ...
                #thumbnail
                 $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $blogpost->ID ), 'clinika_portfolio_pic400x400' );
    ...
    $class_slider = 'mt_slider_members_'.uniqid();
        ...
        $members = get_posts($args_members);
            foreach ($members as $member) {
                #metaboxes
                $metabox_member_position = get_post_meta( $member->ID, 'clinika_member_position', true );
                $metabox_member_email = get_post_meta( $member->ID, 'clinika_member_email', true );
                $metabox_member_phone = get_post_meta( $member->ID, 'clinika_member_phone', true );

                $metabox_facebook_profile = get_post_meta( $member->ID, 'clinika_facebook_profile', true );
                $metabox_twitter_profile  = get_post_meta( $member->ID, 'clinika_twitter_profile', true );
                $metabox_linkedin_profile = get_post_meta( $member->ID, 'clinika_linkedin_profile', true );
                $metabox_vimeo_url = get_post_meta( $member->ID, 'clinika_vimeo_url', true );
                ...
                $content        = str_replace(']]>', ']]&gt;', $content);
                #thumbnail
                $thumbnail_src = wp_get_attachment_image_src( get_post_thumbnail_id( $member->ID ),'clinika_500x500' );
                                ...
                                <div class="members_img_holder">
                                    ...
                                    <div class="member01-content">
                                        <div class="member01-content-inside">
                                            <h4 class="member01_name"><a href="'.get_permalink($member->ID).'">'.$member->post_title.'</a></h4>
...
// Shortcode : Blood Requests //
function mt_blood_single_request($params,  $content = NULL) {
    extract( shortcode_atts( 
        array(
          'number_columns'     => ''
        ), $params ) );
    ...
    $html = '';
      $html .= '<div class="blood-request-wrapper row">';
        $html .= '<div class="blood-request-content mt_'.esc_attr($number_columns).'">';       
...
add_shortcode('mt_blood_single_request_short', 'mt_blood_single_request');
/**
||-> Shortcode: Child Shortcode v1
*/
function mt_blood_single_request_items($params, $content = NULL) {
    ...
    $html = '';
      $html .= '<ul class="single-request-wrapper">'; 
        $html .= '<li class="single-requests">';

        $html .= '<div class="request_infos" style="background: url('.$banner[0].') no-repeat center center;border-top: 3px solid '.$border_color.'">';
           $html .= '<h3 class="mt-title"><a href="'.esc_url($banner_button_url).'">'.esc_attr($additional_title).'</a></h3>';
           $html .= '<div class="request-svg">
                        <img src="'.plugins_url( 'images/blood-drop.svg', __FILE__ ).'">
                    </div>';
          $html .= '<ul>';
            if($banner_group) {
                $html .= '<li>'.esc_html__('Blood Group :','modeltheme').'<span class="info-right">'.esc_attr($banner_group).'</span></li>';
            }
            if($banner_unit) {
                $html .= '<li>'.esc_html__('Blood Unit :','modeltheme').'<span class="info-right">'.esc_attr($banner_unit).'</span></li>';
            }
            if($banner_when) { 
                $html .= '<li>'.esc_html__('When Needed :','modeltheme').'<span class="info-right">'.esc_attr($banner_when).'</span></li>';
            }
            if($banner_nr) { 
                $html .= '<li>'.esc_html__('Number :','modeltheme').'<span class="info-right">'.esc_attr($banner_nr).'</span></li>';
            }
          $html .= '</ul>';

          $html .= '<div class="request-btn">';
                $html .= '<a href="'.esc_url($banner_button_url).'"><i class="fa fa-angle-right"></i></a>';
          $html .= '</div>'; 
...
add_shortcode('mt_blood_single_request_short_item', 'mt_blood_single_request_items');
    ...
    function mt_icon_services_vc_element() {
        if (function_exists('vc_map')) {
            vc_map( array(
                ...
                "category" => esc_attr__('Clinika Theme', 'modeltheme'),
                "params" => array(
